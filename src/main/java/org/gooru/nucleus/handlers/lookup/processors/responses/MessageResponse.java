...
public final class MessageResponse {
  ...
  private MessageResponse(JsonObject response) {
    String statusString = response.getString(MessageConstants.MSG_OP_STATUS);
    this.deliveryOptions = new DeliveryOptions().addHeader(MessageConstants.MSG_OP_STATUS, statusString);
    this.reply = response.getJsonObject(MessageConstants.RESP_CONTAINER_MBUS);
    this.status = MessageConstants.MSG_OP_STATUS_SUCCESS.equalsIgnoreCase(statusString);
  }
  ...
  public JsonObject getSuccessResult() {
    return this.reply.getJsonObject(MessageConstants.MSG_HTTP_BODY).getJsonObject(MessageConstants.MSG_HTTP_RESPONSE);
  }
